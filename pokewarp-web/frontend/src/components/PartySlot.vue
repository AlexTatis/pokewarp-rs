...
<template>
    <div class="w-96 h-32 shadow-lg rounded-full bg-white" :class="{' border-red-500 border-dashed border-2': selected}" @click="selected = !selected">
        <div v-if="PROPS.pkm.id != 0" class="flex flex-row items-center h-full pixelated gap-2 pl-2">
            <img :src="`/pokesprite/pokemon-gen8/regular/${PROPS.pkm.species.toLowerCase()}.png`" alt="" class="w-28 animate-bounce">
            <div>
                <p class="text-3xl font-bold">{{ PROPS.pkm.nickname }}</p>
                <p class="font-semibold">Lvl. {{ PROPS.pkm.level }}</p>
            </div>
            <div class="tooltip m-auto" :data-tip="pkm.item.name" v-if="pkm.item.num">
